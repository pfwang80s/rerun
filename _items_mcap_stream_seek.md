...
| M2 | MCAP-013…033 | 15/21 | 进行中 | 不接 TimeControl 即可安全完成 metadata opening、bounded decoder initialization、局部 Chunk 验证和 terminal batch 派生 |
...
有效工作项总数为 88，当前进度为 27/88；26 个 `[~]` 项不计入分母且不产生提交。
...
- 依赖：MCAP-009、MCAP-021、MCAP-023、MCAP-029。
...
### [ ] MCAP-048 — 实现唯一 public lifecycle sequencer
...
### [ ] MCAP-049 — 实现 Rust lifecycle delivery permits
...
- 变更：在全部required release-Wasm与对抗evidence满足后，生成并checked-in唯一canonical V1 profile artifact，复核schema fingerprint/units/scopes/accounting/provenance，并在 `re_web`模块内安装唯一private constructor以签发one-shot Wasm remote-MCAP production capability；tooling draft、validated artifact和test profile保持disarmed。
...
- 验收：artifact symbol/dependency/API gate证明只有Web remote-MCAP root能取得one-shot production capability和bounded intern transaction，旧constructor仍存在且native及全部nonremote Web routes未被迁移或取得accounting root。
...
| ROS 2/protobuf bounded initializer、decoder assignment、两阶段decode与manifest | MCAP-026 至 MCAP-033 |
