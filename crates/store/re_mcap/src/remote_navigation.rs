...
impl RemoteNavigationAdapterV1 {
    ...
    fn with_extent_and_play_state_v1(
        ...
        if matches!(indexed_extent, CanonicalIndexedExtentV1::NoIndexedMessages)
            && initial_play_state != RemotePlayStateV1::Paused
        {
    ...
    fn refresh_ui_state_v1(&mut self, clock_held: bool) {
        ...
        self.cached_ui_state = RemoteNavigationUiStateV1 {
            ...
            committed: self.committed,
            target_marker,
            is_loading: self.requested.is_some_and(|intent| {
                Some(intent.canonical_target) != self.committed.map(|c| c.cursor)
            }),
...
mod tests {
    ...
    fn commit_updates_only_committed_time_and_clears_latches() {
        ...
        adapter
            .commit_presentation_v1(Some(committed_at(20)))
            .expect("commit should succeed");

        assert_eq!(adapter.committed_time_v1(), Some(committed_at(20)));
        assert_eq!(
            adapter.requested_v1().map(|intent| intent.canonical_target),
            Some(TimeInt::new_temporal(30))
        );
        ...
        assert_eq!(
            adapter.committed_time_v1().map(|time| time.cursor),
