...
impl RemoteNavigationCommandV1 {
    pub(crate) const fn timeline_v1(self) -> Timeline {
        match self {
            ...
            | Self::SetPlaybackSpeed { timeline, .. } => timeline,
        }
    }
...
pub(crate) enum RemoteNavigationErrorV1 {
    ...
    #[error("remote navigation received an empty or inverted indexed extent")]
    InvalidIndexedExtent,
...
impl RemoteNavigationAdapterV1 {
    ...
    pub(crate) fn preview_update_v1(
        ...
        if prospective.accepted_discrete_command_this_frame {
            ...
            prospective.requested = Some(PendingNavigationIntentV1::new_v1(
                target,
                prospective.play_state,
    ...
    /// Commits a presentation cursor.
    ///
    /// The caller must invoke this at the commit point of the current frame: after this frame's
    /// `preview_update_v1` and before the query phase. Generation correlation and stale-commit
    /// protection belong to the MCAP-094 `CommitLocked(generation, T2)` coordinator, not to this
    /// pure state layer.
    pub(crate) fn commit_presentation_v1(
        &mut self,
        committed: Option<CommittedPresentationTimeV1>,
    ) -> Result<&RemoteNavigationUiStateV1, RemoteNavigationErrorV1> {
        validate_committed_time_v1(self.canonical_timeline, self.indexed_extent, committed)?;
    ...
    fn apply_merge_command_v1(
        ...
        match command {
            RemoteNavigationCommandV1::Seek { target, .. } => {
                ...
                self.set_requested_target_v1(target, RemoteNavigationTriggerV1::Seek)?;
                self.target_marker = Some(target);
                self.accepted_discrete_command_this_frame = true;
            }
            RemoteNavigationCommandV1::LoopJump { target, .. } => {
                self.ensure_target_in_extent_v1(target)?;
                self.set_requested_target_v1(target, RemoteNavigationTriggerV1::LoopJump)?;
                self.target_marker = Some(target);
                self.accepted_discrete_command_this_frame = true;
            }
            RemoteNavigationCommandV1::StepForward { delta, .. } => {
                self.apply_step_command_v1(delta, true)?;
                self.accepted_discrete_command_this_frame = true;
            }
            RemoteNavigationCommandV1::StepBack { delta, .. } => {
                self.apply_step_command_v1(delta, false)?;
                self.accepted_discrete_command_this_frame = true;
            }
    ...
    fn apply_step_command_v1(
        ...
        let base = i128::from(self.step_base_target_v1()?.as_i64());
    ...
    fn set_requested_target_v1(
        ...
        self.requested = Some(PendingNavigationIntentV1::new_v1(
            target,
            self.play_state,
            trigger,
        )?);
...
fn validate_indexed_extent_v1(
    ...
    match indexed_extent {
        CanonicalIndexedExtentV1::Known(range) if range.is_empty() => {
            Err(RemoteNavigationErrorV1::InvalidIndexedExtent)
        }
        CanonicalIndexedExtentV1::NoIndexedMessages if play_state.is_playing_v1() => {
            Err(RemoteNavigationErrorV1::NoIndexedMessages)
        }
        CanonicalIndexedExtentV1::Known(_) | CanonicalIndexedExtentV1::NoIndexedMessages => Ok(()),
    }
}
...
fn bound_candidate_v1(
    ...
    let bound = match loop_mode {
        RemoteLoopModeV1::Clamp => candidate.clamp(min, max),
...
mod tests {
    ...
    fn commit_updates_only_committed_time_and_clears_latches() {
        ...
        adapter
            .commit_presentation_v1(Some(committed_at(30)))
    ...
    fn same_cursor_play_state_change_is_a_pending_demand() {
        ...
        adapter
            .commit_presentation_v1(Some(committed_at(10)))
        ...
        assert!(
            adapter
                .requested_v1()
                .expect("play creates a requested demand")
                .demand_key
    ...
    fn inverted_extent_is_rejected_atomically_before_clamp() {
        ...
        assert_eq!(error, RemoteNavigationErrorV1::InvalidIndexedExtent);
        assert_eq!(adapter, before);
    }

    #[test]
    fn commit_after_preview_resumes_next_frame() {
        let mut adapter = playing_adapter();
        adapter
            .merge_commands_v1(&command(RemoteNavigationCommandV1::Seek {
                timeline: canonical_timeline(),
        ...
        // Commit-frame preview runs after the discrete seek and must not auto-advance.
        adapter
            .preview_update_v1(extent(), RemoteLoopModeV1::Clamp, clock(100, false))
            .expect("commit-frame preview should succeed");
        assert_eq!(
            adapter.requested_v1().map(|intent| intent.canonical_target),
            Some(TimeInt::new_temporal(30))
        );

        // Commit at the documented commit point (after preview, before query).
        adapter
            .commit_presentation_v1(Some(committed_at(30)))
            .expect("commit should succeed");
        assert_eq!(adapter.requested_v1(), None);

        // Next frame resumes integration from the new committed cursor.
        adapter
            .preview_update_v1(extent(), RemoteLoopModeV1::Clamp, clock(5, false))
    ...
    fn frozen_release_suppresses_one_preview_dt() {
        ...
        adapter
            .preview_update_v1(extent(), RemoteLoopModeV1::Clamp, clock(100, false))
            .expect("recovery preview should suppress the dt");
        assert_eq!(adapter.requested_v1(), None);

        adapter
            .preview_update_v1(extent(), RemoteLoopModeV1::Clamp, clock(5, false))
            .expect("post-recovery preview should integrate");
        assert_eq!(
            adapter.requested_v1().map(|intent| intent.canonical_target),
            Some(TimeInt::new_temporal(15))
        );
    ...
    #[test]
    fn in_flight_release_suppresses_one_preview_dt() {
        let mut adapter = playing_adapter();
        adapter.set_requested_generation_in_flight_v1(true);
        ...
        adapter
            .preview_update_v1(extent(), RemoteLoopModeV1::Clamp, clock(100, false))
            .expect("release preview should suppress the dt");
        assert_eq!(adapter.requested_v1(), None);

        adapter
            .preview_update_v1(extent(), RemoteLoopModeV1::Clamp, clock(5, false))
            .expect("post-release preview should integrate");
        assert_eq!(
            adapter.requested_v1().map(|intent| intent.canonical_target),
            Some(TimeInt::new_temporal(15))
        );
    ...
    fn checked_error_paths_leave_state_unchanged() {
        ...
        let before = adapter.clone();
        let error = adapter
            .commit_presentation_v1(Some(CommittedPresentationTimeV1::new_v1(
                other_timeline(),
